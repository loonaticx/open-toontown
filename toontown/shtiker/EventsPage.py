...
from pandac.PandaModules import Vec4, Vec3, TextNode, PNMImage, StringStream, Texture, HTTPClient, DocumentSpec, \
    Ramfile, Point3
...
class EventsPage(ShtikerPage.ShtikerPage):
    ...
    def load(self):
        ...
        self.loadNewsTab()
        self.titleLabel = DirectLabel(parent = self, relief = None, text = TTLocalizer.EventsPageHostTabTitle,
                                      text_scale = TTLocalizer.EPtitleLabel, textMayChange = True,
    ...
    def loadTabs(self):
        ...
        self.hostTab = DirectButton(parent = self, relief = None, text = TTLocalizer.EventsPageHostTabName,
                                    text_scale = TTLocalizer.EPhostTab, text_align = TextNode.ACenter,
                                    text_pos = (0.12, 0.0), image = gui.find('**/tabs/polySurface1'),
                                    ...
                                    extraArgs = [EventsPage_Host], pos = (0.92, 0, 0.55))
        self.invitedTab = DirectButton(parent = self, relief = None, text = TTLocalizer.EventsPageInvitedTabName,
                                       text_scale = TTLocalizer.EPinvitedTab, text_pos = (0.12, 0.0),
                                       text_align = TextNode.ACenter, image = gui.find('**/tabs/polySurface2'),
                                       image_pos = (0.12, 1, -0.91), image_hpr = (0, 0, -90),
                                       image_scale = (0.033, 0.033, 0.035), image_color = normalColor,
                                       image1_color = clickColor, image2_color = rolloverColor,
                                       image3_color = diabledColor, text_fg = Vec4(0.2, 0.1, 0, 1),
                                       command = self.setMode, extraArgs = [EventsPage_Invited], pos = (0.92, 0, 0.1))
        self.calendarTab = DirectButton(parent = self, relief = None, text = TTLocalizer.EventsPageCalendarTabName,
                                        text_scale = TTLocalizer.EPcalendarTab, text_pos = (0.12, 0.0),
                                        text_align = TextNode.ACenter, image = gui.find('**/tabs/polySurface2'),
                                        image_pos = (0.12, 1, -0.91), image_hpr = (0, 0, -90),
                                        image_scale = (0.033, 0.033, 0.035), image_color = normalColor,
                                        image1_color = clickColor, image2_color = rolloverColor,
                                        image3_color = diabledColor, text_fg = Vec4(0.2, 0.1, 0, 1),
                                        command = self.setMode, extraArgs = [EventsPage_Calendar], pos = (0.92, 0, 0.1))
        self.newsTab = DirectButton(parent = self, relief = None, text = TTLocalizer.EventsPageNewsTabName,
                                    text_scale = TTLocalizer.EPnewsTab, text_pos = (0.12, 0.0),
                                    text_align = TextNode.ACenter, image = gui.find('**/tabs/polySurface2'),
                                    image_pos = (0.12, 1, -0.91), image_hpr = (0, 0, -90),
                                    image_scale = (0.033, 0.033, 0.035), image_color = normalColor,
                                    image1_color = clickColor, image2_color = rolloverColor,
                                    image3_color = diabledColor, text_fg = Vec4(0.2, 0.1, 0, 1), command = self.setMode,
                                    extraArgs = [EventsPage_News], pos = (0.92, 0, 0.1))
        self.newsTab.hide()
        if self.UseNewsTab:
            self.newsTab.show()
    ...
    def loadHostingTab(self):
        ...
        self.hostingBackgroundFlat = DirectFrame(parent = self.hostedPartyDisplay, relief = None,
                                                 geom = self.hostingGui.find('**/background_flat'))
        self.hostingGuestList, self.hostingGuestLabel = self.createListAndLabel(self.hostedPartyDisplay,
                                                                                self.hostingGui, 'guests', 7)
        self.hostingActivityList, self.hostingActivityLabel = self.createListAndLabel(self.hostedPartyDisplay,
                                                                                      self.hostingGui, 'activities', 1)
        self.hostingDecorationList, self.hostingDecorationLabel = self.createListAndLabel(self.hostedPartyDisplay,
                                                                                          self.hostingGui,
                                                                                          'decorations', 1)
        self.hostingDateLabel = DirectLabel(parent = self.hostedPartyDisplay, relief = None, text = '',
                                            scale = TTLocalizer.EPhostingDateLabel, text_align = TextNode.ACenter,
                                            text_wordwrap = 10, textMayChange = True,
                                            pos = self.hostingGui.find('**/date_locator').getPos())
        pos = self.hostingGui.find('**/cancel_text_locator').getPos()
        self.hostingCancelButton = DirectButton(parent = self.hostedPartyDisplay, relief = None,
                                                ...
                                                text_pos = (pos[0], pos[2]), command = self.__doCancelParty)
        pos = self.hostingGui.find('**/startParty_text_locator').getPos()
        self.partyGoButton = DirectButton(parent = self.hostedPartyDisplay, relief = None,
                                          ...
                                          text_scale = TTLocalizer.EPpartyGoButton, text_pos = (pos[0], pos[2]),
                                          textMayChange = True, command = self._startParty)
        self.publicPrivateLabel = DirectLabel(parent = self.hostedPartyDisplay, relief = None,
        ...
        self.publicButton = DirectCheckButton(parent = self.hostedPartyDisplay, relief = None, scale = 0.1,
                                              ...
                                              text_align = TextNode.ALeft, text_scale = TTLocalizer.EPpublicButton,
        ...
        self.confirmCancelPartyGui = TTDialog.TTGlobalDialog(dialogName = self.uniqueName('confirmCancelPartyGui'),
        ...
        self.confirmTooLatePartyGui = TTDialog.TTGlobalDialog(dialogName = self.uniqueName('confirmTooLatePartyGui'),
        ...
        self.confirmPublicPrivateGui = TTDialog.TTGlobalDialog(dialogName = self.uniqueName('confirmPublicPrivateGui'),
        ...
        self.cancelPartyResultGui = TTDialog.TTGlobalDialog(dialogName = self.uniqueName('cancelPartyResultGui'),
    ...
    def loadInvitationsTab(self):
        ...
        self.invitationBackgroundFlat = DirectFrame(parent = self.invitationDisplay, relief = None,
                                                    geom = self.invitationGui.find('**/background_flat'))
        self.invitationPartiesFlat = DirectFrame(parent = self.invitationDisplay, relief = None,
                                                 geom = self.invitationGui.find('**/parties_background'))
        self.invitationActivtiesFlat = DirectFrame(parent = self.invitationDisplay, relief = None,
                                                   geom = self.invitationGui.find('**/activities_background'))
        self.invitationPartyList, self.invitationPartyLabel = self.createListAndLabel(self.invitationDisplay,
                                                                                      self.invitationGui, 'parties', 7,
                                                                                      'ButtonDown', 'ButtonUp',
                                                                                      'Text_locator')
        self.invitationActivityList, self.invitationActivityLabel = self.createListAndLabel(self.invitationDisplay,
                                                                                            self.invitationGui,
                                                                                            'activities', 1,
                                                                                            'ButtonDown', 'ButtonUp',
                                                                                            'Text_locator')
        pos = self.invitationGui.find('**/startText_locator').getPos()
        self.invitePartyGoButton = DirectButton(parent = self.invitationDisplay, relief = None,
                                                ...
                                                text_pos = (pos[0], pos[2]), textMayChange = True,
                                                command = self._inviteStartParty)
        self.invitationDateTimeLabel = DirectLabel(parent = self.invitationDisplay, relief = None, text = '',
                                                   textMayChange = True, text_scale = 0.07, pos = (0, 0, -0.65))
        return

    def loadCalendarTab(self):
        self.calendarDisplay = self.attachNewNode('calendar')
        self.toontownTimeLabel = DirectLabel(parent = self.calendarDisplay, pos = (0.175, 0, -0.69),
                                             text_align = TextNode.ARight, relief = None,
                                             text = TTLocalizer.EventsPageToontownTimeIs, text_scale = 0.065,
                                             text_font = ToontownGlobals.getMinnieFont(), text_fg = (255 / 255.0,
                                                                                                     146 / 255.0,
                                                                                                     113 / 255.0,
                                                                                                     1),
                                             textMayChange = 0)
    ...
    def loadNewsTab(self):
        ...
        self.newsStatusLabel = DirectLabel(text = TTLocalizer.EventsPageNewsDownloading, relief = None,
                                           text_scale = 0.1, text_wordwrap = 13, parent = self.newsFrame,
                                           pos = (0, 0, 0.275))
        self.createArticleIndexList()
        titlePos = self.newsFrame.find('**/loc_toontimeTimes').getPos()
        self.newsPaperTitle = DirectLabel(text = TTLocalizer.EventsPageNewsPaperTitle, relief = None,
                                          text_scale = (0.13, 0.25, 1), text_align = TextNode.ACenter,
                                          text_font = ToontownGlobals.getMinnieFont(), parent = self.newsFrame,
                                          pos = titlePos)
        subLeftPos = self.newsFrame.find('**/loc_subheaderLf').getPos()
        subRightPos = self.newsFrame.find('**/loc_subheaderRt').getPos()
        self.subLeft = DirectLabel(text = TTLocalizer.EventsPageNewsLeftSubtitle, relief = None, text_scale = 0.05,
                                   text_align = TextNode.ALeft, parent = self.newsFrame, pos = subLeftPos)
        self.subRight = DirectLabel(text = TTLocalizer.EventsPageNewsRightSubtitle, relief = None, text_scale = 0.05,
                                    text_align = TextNode.ARight, parent = self.newsFrame, pos = subRightPos)
        if self.UseNewsTab:
            self.downloadArticles()
        return

    def getGuestItem(self, name, inviteStatus):
        label = DirectLabel(relief = None, text = name, text_scale = 0.045, text_align = TextNode.ALeft,
                            textMayChange = True)
    ...
    def getActivityItem(self, activityBase, count = 1):
        ...
        label = DirectLabel(relief = None, geom = geom, geom_scale = 0.38, geom_pos = Vec3(0.0, 0.0, -0.17),
                            text = textForActivity, text_scale = TTLocalizer.EPactivityItemLabel,
                            text_align = TextNode.ACenter, text_pos = (-0.01, -0.43), text_wordwrap = 7.0)
        return label
    ...
    def getDecorationItem(self, decorBase, count = 1):
        ...
        label = DirectLabel(relief = None, geom = self.decorationModels.find('**/partyDecoration_%s' % assetName),
                            text = textForDecoration, text_scale = TTLocalizer.EPdecorationItemLabel,
                            text_align = TextNode.ACenter, text_pos = (-0.01, -0.43), text_wordwrap = 7.0)
    ...
    def loadInvitations(self):
        ...
        for partyInfo in base.localAvatar.partiesInvitedTo:
            if partyInfo.status == PartyGlobals.PartyStatus.Cancelled or partyInfo.status == \
            ...
            item = DirectButton(relief = None, text = hostName, text_align = TextNode.ALeft,
                                text_bg = Vec4(0.0, 0.0, 0.0, 0.0), text_scale = 0.045, textMayChange = True,
    ...
    def fillInviteActivityList(self, activityIds):
        ...
        for activityId in countDict:
            ...
            item = DirectLabel(relief = None, text = textOfActivity, text_align = TextNode.ACenter, text_scale = 0.05,
                               text_pos = (0.0, -0.15), geom_scale = 0.3, geom_pos = Vec3(0.0, 0.0, 0.07), geom = geom)
    ...
    def loadHostedPartyInfo(self):
        ...
        if base.localAvatar.hostedParties is not None and len(base.localAvatar.hostedParties) > 0:
            for partyInfo in base.localAvatar.hostedParties:
                if partyInfo.status == PartyGlobals.PartyStatus.Pending or partyInfo.status == \
                        PartyGlobals.PartyStatus.CanStart or partyInfo.status == \
                        PartyGlobals.PartyStatus.NeverStarted or partyInfo.status == PartyGlobals.PartyStatus.Started:
                    ...
                    if partyInfo.status == PartyGlobals.PartyStatus.CanStart:
                        ...
                        if isinstance(place, Party):
                            if hasattr(base, 'distributedParty'):
                                ...
                                self.partyGoButton['state'] = DirectGuiGlobals.NORMAL
                                self.notify.warning(
                                    'base.distributedParty is not defined when base.cr.playGame.getPlace is party. '
    ...
    def checkCanStartHostedParty(self):
        result = True
        if self.hostedPartyInfo.endTime < base.cr.toontownTimeManager.getCurServerDateTime() and \
    ...
    def createListAndLabel(self, parent, gui, typeString, itemsVisible, downString = 'DownArrow', upString = 'UpArrow',
                           textString = '_text_locator'):
        list = DirectScrolledList(parent = parent, relief = None,
                                  ...
                                  itemFrame_relief = None, numItemsVisible = itemsVisible, forceHeight = 0.07)
        strings = {
        ...
        label = DirectLabel(parent = parent, relief = None, text = strings[typeString],
    ...
    def __doCancelParty(self):
        if self.hostedPartyInfo:
            if self.hostedPartyInfo.status == PartyGlobals.PartyStatus.Pending or self.hostedPartyInfo.status == \
                    PartyGlobals.PartyStatus.CanStart or self.hostedPartyInfo.status == \
    ...
    def confirmCancelOfParty(self):
        ...
        if self.confirmCancelPartyGui.doneStatus == 'ok':
            base.cr.partyManager.sendChangePartyStatusRequest(self.hostedPartyInfo.partyId,
    ...
    def changePartyStatusResponseReceived(self, partyId, newPartyStatus, errorCode, beansRefunded):
        ...
        elif errorCode == PartyGlobals.ChangePartyFieldErrorCode.AlreadyRefunded and newPartyStatus == \
                PartyGlobals.PartyStatus.NeverStarted:
            self.loadHostedPartyInfo()
    ...
    def createArticleTextList(self):
        ...
        self.articleTextList = DirectScrolledList(parent = self.newsFrame, relief = None, pos = topLeftStart,
                                                  ...
                                                  incButton_relief = None, incButton_pos = incButtonPos,
                                                  ...
                                                  decButton_relief = None, decButton_pos = decButtonPos,
                                                  decButton_image3_color = (1.0, 1.0, 1.0, 0.1), text_scale = 0.05,
                                                  ...
                                                  frameColor = (0.82, 0.8, 0.75, 1), borderWidth = (0.01, 0.01),
                                                  numItemsVisible = self.numLinesInTextList,
                                                  itemMakeFunction = makeButton, forceHeight = itemHeight)
        oldParent = self.articleTextList.decButton.getParent()
        self.newsFrame.find('**/scroll').hide()
        return

    def createArticleIndexList(self):
        self.articleIndexList = DirectScrolledList(parent = self.newsFrame, relief = None, pos = (0, 0, 0),
                                                   ...
                                                   decButton_scale = 1, text_scale = 0.05, numItemsVisible = 1)
    ...
    def getRssFeed(self):
        ...
        def addFeedText(unicodeText, textSize = 0.03, color = (0, 0, 0, 1), feedText = feedText):
            feedText.append(
                DirectLabel(relief = None, text = unicodeText, text_scale = textSize, text_align = TextNode.ALeft,
                            text_fg = color, textMayChange = 0, pos = (0, 0, 0)))
            return
    ...
    def displayArticleImage(self, img):
        ...
        self.articleImage = DirectFrame(parent = self.newsFrame, relief = DirectGuiGlobals.FLAT, image = webTexture,
                                        image_scale = (curXSize / 2.0, 1, curYSize / 2.0), pos = center,
                                        frameSize = (minX,
                                                     maxX,
                                                     minY,
                                                     maxY))
    ...
    def displayArticleText(self, articleText):
        playaLabel = DirectLabel(parent = None, relief = None, text_align = TextNode.ALeft, text = articleText,
                                 text_scale = 0.06, text_wordwrap = 13.5)
        playaLabel.hide()
    ...
    def createNewsList(self):
        ...
        self.newsList = DirectScrolledList(parent = self.newsFrame, items = self.feedText, relief = None,
                                           pos = (-0.5, 0, 0.4),
                                           ...
                                           incButton_relief = None, incButton_scale = (
            self.newsArrowButtonScale, self.newsArrowButtonScale, -self.newsArrowButtonScale), incButton_pos = (
            self.newsButtonXstart, 0, self.newsListZorigin - buttonOffSet), incButton_image3_color = Vec4(1, 1, 1, 0.2),
                                           decButton_image = (self.scrollButtonGui.find('**/FndsLst_ScrollUp'),
                                                              self.scrollButtonGui.find('**/FndsLst_ScrollDN'),
                                                              self.scrollButtonGui.find('**/FndsLst_ScrollUp_Rllvr'),
                                                              self.scrollButtonGui.find('**/FndsLst_ScrollUp')),
                                           decButton_relief = None, decButton_scale = (
            self.newsArrowButtonScale, self.newsArrowButtonScale, self.newsArrowButtonScale), decButton_pos = (
            self.newsButtonXstart, 0, self.newsListZorigin + self.newsListFrameSizeZ + buttonOffSet),
                                           decButton_image3_color = Vec4(1, 1, 1, 0.2),
                                           itemFrame_pos = (self.newsItemFrameXorigin, 0, 0), itemFrame_scale = 1.0,
                                           itemFrame_relief = DirectGuiGlobals.SUNKEN,
                                           itemFrame_frameSize = (self.newsListXorigin,
