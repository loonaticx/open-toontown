...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI, SuitPlannerBase.SuitPlannerBase):
    ...
    SuitHoodInfo = [
        ...
        [2200, 3, 10, 0, 5, 15, 3, (1, 5, 10, 40, 60, 80), (10, 70, 10, 10), (1, 2, 3), []],
        [2300, 3, 10, 0, 5, 15, 3, (1, 5, 10, 40, 60, 80), (10, 10, 40, 40), (1, 2, 3), []],
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover = None, cogdoTakeover = None,
                      minPathLen = None, maxPathLen = None, buildingHeight = None, suitLevel = None, suitType = None,
                      suitTrack = None, suitName = None, skelecog = None, revives = None):
        startPoint = None
        blockNumber = None
        if self.notify.getDebug():
        ...
        gotDestination = self.chooseDestination(newSuit, startTime, toonBlockTakeover = toonBlockTakeover,
    ...
    def chooseDestination(self, suit, startTime, toonBlockTakeover = None, cogdoTakeover = None, minPathLen = None,
                          maxPathLen = None):
        possibles = []
        backup = []
        if cogdoTakeover is None:
    ...
    def upkeepSuitPopulation(self, task):
        ...
        if self.notify.getDebug() and self.currDesired == None:
            ...
            self.zoneId, self.numFlyInSuits, targetFlyInNum, self.numBuildingSuits, targetBuildingNum))
        ...
        if self.buildingMgr:
            ...
            if timeout != None:
                ...
                if oldestAge > timeout:
                    self.notify.info('Street %d has %d buildings; reclaiming %0.2f-hour-old building.' % (
    ...
    def assignInitialSuitBuildings(self):
        ...
        if simbase.air.wantCogdominiums:
            ...
            wantedCogdos = 0
        self.notify.debug('Want %d out of %d total suit buildings; we currently have %d assigned, %d actual.' % (
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            self.formatNumSuitsPerTrack(numPerTrack), self.formatNumSuitsPerTrack(numPerHeight), smallestTracks,
            smallestHeights, buildingTrack, buildingHeight))
            repeat = 1
            while repeat and buildingTrack != None and buildingHeight != None:
                if len(hoodInfo) == 0:
                    self.notify.warning(
                        'No more streets can have suit buildings, with %d buildings unassigned!' % numToAssign)
                    return
                repeat = 0
                ...
                if zoneId in self.air.suitPlanners:
                    ...
                    numBldgs = 0
                    numTotalBuildings = 0
                if numCogdos + numBldgs >= currHoodInfo[
                    self.SUIT_HOOD_INFO_BMAX] or numCogdos + numBldgs >= numTotalBuildings:
            ...
            if buildingTrack != None and buildingHeight != None:
                ...
                self.notify.info('Assigning building to zone %d, pending tracks = %s, pending heights = %s' % (
    ...
    def unassignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            while repeat:
                if len(hoodInfo) == 0:
                    self.notify.warning(
                        'No more streets can remove suit buildings, with %d buildings too many!' % numToAssign)
                    return
                repeat = 0
    ...
    def requestBattle(self, zoneId, suit, toonId):
        ...
        if simbase.config.GetBool('props-buff-battles', True) and canonicalZoneId in self.cellToGagBonusDict:
            ...
            if tentativeBonusTrack in trackToHolidayDict:
                ...
                if simbase.air.holidayManager.isHolidayRunning(holidayId) and simbase.air.holidayManager.getCurPhase(
                        holidayId) >= 1:
                    interactivePropTrackBonus = tentativeBonusTrack
        self.battleMgr.newBattle(zoneId, zoneId, pos, suit, toonId, self.__battleFinished,
    ...
    def postBattleResumeCheck(self, suit):
        self.notify.debug(
            'DistSuitPlannerAI:postBattleResumeCheck:  suit ' + str(suit.getDoId()) + ' is leaving battle')
        battleIndex = 0
