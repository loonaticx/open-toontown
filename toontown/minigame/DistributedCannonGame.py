...
class DistributedCannonGame(DistributedMinigame):
    ...
    def __calcFlightResults(self, avId, launchTime):
        ...
        self.notify.debug(
            'calcFlightResults(%s): rotation(%s), angle(%s), horizVel(%s), xVel(%s), yVel(%s), zVel(%s), '
    ...
    def __flyTask(self, task):
        ...
        if t == task.info['timeOfImpact']:
            ...
            if task.info['hitWhat'] == self.HIT_WATER:
                ...
                s = Sequence(Wait(0.5),
                             toon.posInterval(duration = LAND_TIME - 0.5, pos = hitPos, blendType = 'easeIn'))
    ...
    def __spawnCameraLookAtLerp(self, targetPos, targetLookAt, duration):
        ...
        self.introCameraSeq = camera.posQuatInterval(duration, Point3(targetPos), targetQuat, blendType = 'easeInOut',
