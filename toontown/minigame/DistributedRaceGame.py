...
class DistributedRaceGame(DistributedMinigame):
    ...
    def load(self):
        ...
        for i in range(1, 5):
            ...
            diceButton = DirectButton(image = (button,
                                               button_down,
                                               button_ro,
                                               None), relief = None, pos = (-0.9 + (i - 1) * 0.2, 0.0, -0.85),
                                      scale = 0.25, command = self.handleInputChoice, extraArgs = [i])
            diceButton.hide()
            self.diceButtonList.append(diceButton)

        self.waitingChoicesLabel = DirectLabel(text = TTLocalizer.RaceGameWaitingChoices, text_fg = VBase4(1, 1, 1, 1),
        ...
        self.chanceCardText = OnscreenText('', fg = (1.0, 0, 0, 1), scale = 0.14, font = ToontownGlobals.getSignFont(),
                                           wordwrap = 14, pos = (0.0, 0.2), mayChange = 1)
    ...
    def runInPlace(self, avatar, lane, currentPlace, newPlace, time):
        ...
        runSeq = Sequence(Func(avatar.setAnimState, 'run', 1),
                          avatar.posQuatInterval(time / 3.0, (pos1[0], pos1[1], pos1[2]), pos1Quat,
                                                 other = self.raceBoard),
                          avatar.posQuatInterval(time / 3.0, (pos2[0], pos2[1], pos2[2]), pos2Quat,
                                                 other = self.raceBoard),
                          avatar.posQuatInterval(time / 3.0, (pos3[0], pos3[1], pos3[2]), pos3Quat,
                                                 other = self.raceBoard),
                          Func(stopRun))
