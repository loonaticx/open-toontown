...
class DistributedCogThiefGame(DistributedMinigame):
    ...
    def handleEnterSphere(self, colEntry):
        ...
        self.notify.debug(
            'handleEnterSphere gametime=%s %s into %s' % (self.getCurrentGameTime(), debugFrom[-1], debugInto[-1]))
        intoName = colEntry.getIntoNodePath().getName()
    ...
    def showToonHitBySuit(self, avId, timestamp):
        ...
        spinHTrack = Sequence(LerpHprInterval(geomNode, flyDur, destHpr, startHpr = startHpr),
                              Func(geomNode.setHpr, startHpr), name = toon.uniqueName('hitBySuit-spinH'))
        ...
        spinPTrack = Sequence(LerpHprInterval(rotNode, flyDur, destHpr, startHpr = startHpr),
                              Func(rotNode.setHpr, startHpr), name = toon.uniqueName('hitBySuit-spinP'))
        i = self.avIdList.index(avId)
        soundTrack = Sequence(Func(base.playSfx, self.sndTable['hitBySuit'][i]), Wait(flyDur * (2.0 / 3.0)),
        ...
        preFunc()
        slipBack = Parallel(Sequence(ActorInterval(toon, 'slip-backward', endFrame = 24),
        ...
        hitTrack = Sequence(Parallel(flyTrack, spinHTrack, spinPTrack, soundTrack), slipBack, Func(postFunc),
    ...
    def handleEnterBarrel(self, colEntry):
        ...
        debugFrom = fromName.split('/')
        self.notify.debug(
            'handleEnterBarrel gameTime=%s %s into %s' % (self.getCurrentGameTime(), debugFrom[-1], debugInto[-1]))
        if 'CogThiefSphere' in intoName:
            parts = intoName.split('-')
    ...
    def getTossPieInterval(self, toon, x, y, z, h, p, r, power, beginFlyIval = Sequence()):
        ...
        toss = Track((0, Sequence(Func(toon.setPosHpr, x, y, z, h, p, r), Func(pie.reparentTo, toon.rightHand),
                                  ...
                                  Parallel(ActorInterval(toon, 'throw', startFrame = 48, partName = 'torso'), animPie),
                                  Func(toon.loop, 'neutral'))), (16.0 / 24.0, Func(pie.detachNode)))
        fly = Track((14.0 / 24.0, SoundInterval(sound, node = toon)), (16.0 / 24.0,
                                                                       Sequence(Func(flyPie.reparentTo, render),
                                                                                Func(flyPie.setPosHpr, toon, 0.52, 0.97,
                                                                                     2.24, 0, -45, 0), beginFlyIval,
                                                                                ProjectileInterval(flyPie,
                                                                                                   startVel =
                                                                                                   getVelocity,
                                                                                                   duration = 6),
        ...
        if hasattr(self, 'rewardPanel'):
            ...
            if task.curScore != score:
                ...
                self.jarIval = Parallel(
                    Sequence(self.rewardPanel.scaleInterval(0.15, s * 3.0 / 4.0, blendType = 'easeOut'),
                             self.rewardPanel.scaleInterval(0.15, s, blendType = 'easeIn')),
                    SoundInterval(self.sndRewardTick), name = 'cogThiefGameRewardJarThrob')
                self.jarIval.start()
            task.curScore = score
        else:
            result = Task.done
