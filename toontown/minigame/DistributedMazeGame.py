...
class DistributedMazeGame(DistributedMinigame):
    ...
    def onstage(self):
        ...
        self.goalBar = DirectWaitBar(parent = render2d, relief = DGG.SUNKEN, frameSize = (-0.35,
                                                                                          ...
                                                                                          -0.15,
                                                                                          0.15),
                                     borderWidth = (0.02, 0.02), scale = 0.42,
                                     pos = (0.84, 0, 0.5 - 0.28 * self.numPlayers + 0.05), barColor = (0, 0.7, 0, 1))
    ...
    def enterPlay(self):
        ...
        orthoDrive = OrthoDrive(self.TOON_SPEED, maxFrameMove = self.MAX_FRAME_MOVE,
                                customCollisionCallback = self.__doMazeCollisions, priority = 1)
    ...
    def enterShowScores(self):
        ...
        lerpTrack = Parallel()
        lerpDur = 0.5
        lerpTrack.append(Parallel(LerpPosInterval(self.goalBar, lerpDur, Point3(0, 0, -.6), blendType = 'easeInOut'),
                                  LerpScaleInterval(self.goalBar, lerpDur, Vec3(self.goalBar.getScale()) * 2.0,
                                                    blendType = 'easeInOut')))
        tY = 0.6
        bY = -.05
        lX = -.5
    ...
    def getIntroTrack(self):
        ...
        lerpTrack.append(
            LerpPosHprInterval(iCamParent, lerpDur, pos = Point3(0, 0, 0), hpr = Point3(0, 0, 0), startHpr = startHpr,
                               name = self.uniqueName('introLerpParent')))
        lerpTrack.append(
            LerpPosHprInterval(camera, lerpDur, pos = origCamPos, hpr = origCamHpr, blendType = 'easeInOut',
                               name = self.uniqueName('introLerpCameraPos')))
        base.localAvatar.startLookAround()

        def cleanup(origCamParent = origCamParent, origCamPos = origCamPos, origCamHpr = origCamHpr,
