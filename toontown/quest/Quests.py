...
def npcMatches(toNpcId, npc):
    return toNpcId == npc.getNpcId() or toNpcId == Any or toNpcId == ToonHQ and npc.getHq() or toNpcId == ToonTailor \
           and npc.getTailor()
...
class NewbieQuest:
    ...
    def getString(self, newStr = TTLocalizer.QuestsCogNewNewbieQuestObjective,
...
class CogPartNewbieQuest(CogPartQuest, NewbieQuest):
    ...
    def getString(self):
        return NewbieQuest.getString(self, newStr = TTLocalizer.QuestsCogPartNewNewbieQuestObjective,
...
def chooseMatchingQuest(tier, validQuestPool, rewardId, npc, av):
    ...
    if rewardId == 400 and QuestDict[questsMatchingReward[0]][QuestDictNextQuestIndex] == NA:
        ...
        if notify.getDebug():
            notify.debug(
                'single part track choice quest: %s tier: %s avId: %s trackAccess: %s bestQuest: %s' % (rewardId,
                                                                                                        tier,
                                                                                                        av.getDoId(),
                                                                                                        av.getTrackAccess(),
                                                                                                        bestQuest))
