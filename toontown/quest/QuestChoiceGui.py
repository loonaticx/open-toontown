...
class QuestChoiceGui(DirectFrame):
    ...
    def __init__(self):
        DirectFrame.__init__(self, relief = None, geom = DGG.getDefaultDialogGeom(),
                             geom_color = Vec4(0.8, 0.6, 0.4, 1), geom_scale = (1.85, 1, 0.9), geom_hpr = (0, 0, -90),
                             pos = (-0.85, 0, 0))
        ...
        guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')),
                                         image_scale = (0.7, 1, 1), text = TTLocalizer.QuestChoiceGuiCancel,
                                         text_scale = 0.06, text_pos = (0, -0.02), command = self.chooseQuest,
                                         extraArgs = [0])
