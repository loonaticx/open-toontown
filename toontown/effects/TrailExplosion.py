...
class TrailExplosion(PooledEffect, EffectController):
    ...
    def createTrack(self):
        ...
        for i in range(self.numTrails):
            ...
            vel = Vec3(0, 0, 0)
            if vels:
                vel = Vec3(vels[i][0] + random.randint(-20, 20), vels[i][1] + random.randint(-20, 20),
            ...
            if self.trailEffects[i]:
                ...
                self.trailIval.append(
                    Sequence(Func(self.trailEffects[i].startLoop), Wait(dur), Func(self.trailEffects[i].stopLoop)))
