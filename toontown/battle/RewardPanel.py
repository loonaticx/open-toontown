...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        self.itemFrame = DirectFrame(parent = self, relief = None, text = TTLocalizer.RewardPanelItems,
        ...
        for i in range(ToontownGlobals.MaxQuestCarryLimit):
            label = DirectLabel(parent = self.questFrame, relief = None, pos = (-0.85, 0, -0.1 * i),
                                text = TTLocalizer.RewardPanelQuestLabel % i, text_scale = 0.05,
                                text_align = TextNode.ALeft)
            label.hide()
            self.questLabelList.append(label)

        self.newGagFrame = DirectFrame(parent = self, relief = None, pos = (0, 0, 0.24), text = '',
                                       text_wordwrap = 14.4, text_pos = (0, -0.46), text_scale = 0.06)
        self.endTrackFrame = DirectFrame(parent = self, relief = None, pos = (0, 0, 0.24), text = '',
                                         text_wordwrap = 14.4, text_pos = (0, -0.46), text_scale = 0.06)
        ...
        self.promotionFrame = DirectFrame(parent = self, relief = None, pos = (0, 0, 0.24), text = '',
                                          text_wordwrap = 14.4, text_pos = (0, -0.46), text_scale = 0.06)
        self.trackLabels = []
        self.trackIncLabels = []
        self.trackBars = []
        ...
        for i in range(len(SuitDNA.suitDepts)):
            ...
            self.meritLabels.append(
                DirectLabel(parent = self.gagExpFrame, relief = None, text = deptName, text_scale = 0.05,
                            text_align = TextNode.ARight, pos = (TTLocalizer.RPmeritLabelPosX, 0, -0.09 * i - 0.125),
                            text_pos = (0, -0.02)))
            self.meritIncLabels.append(
                DirectLabel(parent = self.gagExpFrame, relief = None, text = '', text_scale = 0.05,
                            text_align = TextNode.ALeft, pos = (0.7, 0, -0.09 * i - 0.125), text_pos = (0, -0.02)))
        ...
        for i in range(len(ToontownBattleGlobals.Tracks)):
            ...
            self.trackLabels.append(DirectLabel(parent = self.gagExpFrame, relief = None, text = trackName,
                                                text_scale = TTLocalizer.RPtrackLabels, text_align = TextNode.ARight,
                                                pos = (0.13, 0, -0.09 * i), text_pos = (0, -0.02)))
            self.trackIncLabels.append(
                DirectLabel(parent = self.gagExpFrame, relief = None, text = '', text_scale = 0.05,
                            text_align = TextNode.ALeft, pos = (0.65, 0, -0.09 * i), text_pos = (0, -0.02)))
        ...
        self.skipButton = DirectButton(parent = self, relief = None,
                                       ...
                                       text_shadow = Vec4(0, 0, 0, 1), text_pos = TTLocalizer.RPskipPos,
                                       textMayChange = 0, command = self._handleSkip)
        return
    ...
    def getTrackIntervalList(self, toon, track, origSkill, earnedSkill, hasUber, guestWaste = 0):
        ...
        uberSkill = ToontownBattleGlobals.UberSkill + ToontownBattleGlobals.Levels[track][
        ...
        if currentSkill >= uberSkill and not hasUber > 0:
            ...
            intervalList.append(Wait(0.1))
            skillDiff = currentSkill - ToontownBattleGlobals.Levels[track][
                ToontownBattleGlobals.LAST_REGULAR_GAG_LEVEL + 1]
    ...
    def getExpTrack(self, toon, origExp, earnedExp, deathList, origQuestsList, itemList, missedItemList, origMeritList,
                    meritList, partList, toonList, uberEntry, helpfulToonsList, noSkip = False):
