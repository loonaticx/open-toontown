...
class NameShop(StateData.StateData):
    ...
    def __init__(self, makeAToon, doneEvent, avList, index, isPaid):
        ...
        self.fsm = ClassicFSM.ClassicFSM('NameShop', [State.State('Init', self.enterInit, self.exitInit, ['PayState']),
                                                      ...
                                                      State.State('TypeAName', self.enterTypeANameState,
                                                                  self.exitTypeANameState, ['PickAName',
                                                                                            'Approval',
                                                                                            'Accepted',
                                                                                            'Rejected']),
    ...
    def makeLabel(self, te, index, others):
        ...
        if alig == TextNode.ARight:
            ...
            newpos = (0.2, 0, 0)
        df = DirectFrame(state = 'normal', relief = None, text = te, text_scale = TTLocalizer.NSmakeLabel,
    ...
    def enter(self, toon, usedNames, warp):
        ...
        if not self.addedGenderSpecific or self.oldBoy != self.boy:
            ...
            self.listsLoaded = 0
            self.allTitles = [' '] + [
                ' '] + self.nameGen.boyTitles * self.boy + self.nameGen.girlTitles * self.girl + \
                             self.nameGen.neutralTitles
            self.allTitles.sort()
            self.allTitles += [' '] + [' ']
            self.allFirsts = [' '] + [
                ' '] + self.nameGen.boyFirsts * self.boy + self.nameGen.girlFirsts * self.girl + \
            ...
            if not self.addedGenderSpecific:
                ...
                self.namePanel = DirectFrame(parent = aspect2d, image = None, relief = 'flat', state = 'disabled',
                                             pos = (-0.42, 0, -0.15), image_pos = (0, 0, 0.025),
                ...
                self.titleCheck = self.makeCheckBox((-0.615, 0, 0.371), TTLocalizer.TitleCheckBox, (0, 0.25, 0.5, 1),
                                                    self.titleToggle)
                self.firstCheck = self.makeCheckBox((-0.2193, 0, 0.371), TTLocalizer.FirstCheckBox, (0, 0.25, 0.5, 1),
                                                    self.firstToggle)
                self.lastCheck = self.makeCheckBox((0.3886, 0, 0.371), TTLocalizer.LastCheckBox, (0, 0.25, 0.5, 1),
                                                   self.lastToggle)
                del self.circle
    ...
    def makeScrollList(self, gui, ipos, mcolor, nitems, nitemMakeFunction, nitemMakeExtraArgs):
        ...
        it = nitems[:]
        ds = DirectScrolledList(items = it, itemMakeFunction = nitemMakeFunction,
                                itemMakeExtraArgs = nitemMakeExtraArgs, parent = aspect2d, relief = None,
                                command = self.__listsChanged, pos = ipos, scale = 0.6, incButton_image = (self.arrowUp,
                                                                                                           self.arrowDown,
                                                                                                           self.arrowHover,
                                                                                                           self.arrowUp),
                                ...
                                numItemsVisible = 5, forceHeight = TTLocalizer.NSdirectScrolleList)
        return ds

    def makeCheckBox(self, npos, ntex, ntexcolor, comm):
        return DirectCheckButton(parent = aspect2d, relief = None, scale = 0.1, boxBorder = 0.08,
                                 boxImage = self.circle, boxImageScale = 4, boxImageColor = VBase4(0, 0.25, 0.5, 1),
                                 boxRelief = None, pos = npos, text = ntex, text_fg = ntexcolor,
                                 text_scale = TTLocalizer.NSmakeCheckBox, text_pos = (0.2, 0),
    ...
    def makeHighlight(self, npos):
        return DirectFrame(parent = aspect2d, relief = 'flat', scale = (0.552, 0, 0.11), state = 'disabled',
                           frameSize = (-0.07,
                                        0.52,
                                        -0.5,
    ...
    def firstToggle(self, value):
        ...
        if not self.firstActive and not self.lastActive:
            ...
            messenger.send('NameShop-mickeyChange',
                           [[TTLocalizer.MustHaveAFirstOrLast1, TTLocalizer.MustHaveAFirstOrLast2]])
            self.chastise = 1
        self.__listsChanged()
        if self.firstActive:
    ...
    def load(self):
        ...
        self.nameLabel = OnscreenText.OnscreenText(TTLocalizer.PleaseTypeName, parent = aspect2d,
                                                   style = OnscreenText.ScreenPrompt, scale = TTLocalizer.NSnameLabel,
                                                   pos = (-0.0163333, 0.53))
        self.nameLabel.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.nameLabel)
        self.typeNotification = OnscreenText.OnscreenText(TTLocalizer.AllNewNames, parent = aspect2d,
        ...
        self.nameMessages = OnscreenText.OnscreenText(TTLocalizer.NameMessages, parent = aspect2d,
                                                      style = OnscreenText.ScreenPrompt, scale = 0.06,
                                                      pos = (-0.0163333, -0.05))
        self.nameMessages.wrtReparentTo(self.typeNamePanel, sort = 2)
        self.typeANameGUIElements.append(self.nameMessages)
        self.nameEntry = DirectEntry(parent = aspect2d, relief = None, scale = TTLocalizer.NSnameEntry,
                                     entryFont = getToonFont(), width = MAX_NAME_WIDTH, numLines = 2, focus = 0,
                                     cursorKeys = 1, pos = (0.0, 0.0, 0.39), text_align = TextNode.ACenter,
                                     command = self.__typedAName, autoCapitalize = 1)
        ...
        self.submitButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp,
                                         ...
                                         text = TTLocalizer.NameShopSubmitButton, text_scale = 0.06,
                                         text_pos = (0, -0.02), command = self.__typedAName)
        ...
        self.randomButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp,
                                         ...
                                         text = TTLocalizer.ShuffleButton, text_scale = 0.06, text_pos = (0, -0.02),
        ...
        self.typeANameButton = DirectButton(parent = aspect2d, relief = None, image = (self.squareUp,
                                                                                       ...
                                                                                       self.squareHover,
                                                                                       self.squareUp),
                                            image_scale = (1, 1.1, 0.9), pos = (0.0033, 0, -.38833),
                                            scale = (1.2, 1, 1.2), text = TTLocalizer.TypeANameButton,
                                            text_scale = TTLocalizer.NStypeANameButton,
                                            text_pos = TTLocalizer.NStypeANameButtonPos, command = self.__typeAName)
        if base.cr.productName in ['DE', 'BR']:
            self.typeANameButton.hide()
        self.pickANameGUIElements.append(self.typeANameButton)
        self.nameResult = DirectLabel(parent = aspect2d, relief = None, scale = TTLocalizer.NSnameResult,
        ...
        self.payDialog = DirectDialog(dialogName = 'paystate', topPad = 0, fadeScreen = 0.2, pos = (0, 0.1, 0.1),
                                      button_relief = None, text_align = TextNode.ACenter,
                                      text = TTLocalizer.NameShopOnlyPaid, buttonTextList = buttonText,
                                      buttonImageList = buttonImage, image_color = GlobalDialogColor,
                                      buttonValueList = [1, 0], command = self.payAction)
        ...
        self.approvalDialog = DirectDialog(dialogName = 'approvalstate', topPad = 0, fadeScreen = 0.2,
                                           pos = (0, 0.1, 0.1), button_relief = None, image_color = GlobalDialogColor,
                                           text_align = TextNode.ACenter, text = TTLocalizer.NameShopToonCouncil,
    ...
    def rejectName(self, str):
        ...
        self.names[0] = ''
        self.rejectDialog = TTDialog.TTGlobalDialog(doneEvent = 'rejectDone', message = str,
    ...
    def enterAcceptedState(self):
        ...
        self.acceptedDialog = TTDialog.TTGlobalDialog(doneEvent = 'acceptedDone',
    ...
    def enterRejectedState(self):
        ...
        self.rejectedDialog = TTDialog.TTGlobalDialog(doneEvent = 'rejectedDone',
    ...
    def waitForServer(self):
        self.waitForServerDialog = TTDialog.TTDialog(text = TTLocalizer.WaitingForNameSubmission,
