...
class DistributedParty(DistributedObject.DistributedObject):
    ...
    def setPartyInfoTuple(self, partyInfoTuple):
        ...
        base.partyHasJukebox = PartyGlobals.ActivityIds.PartyJukebox in allActIds or \
    ...
    def setPartyStartedTime(self, startedTime):
        ...
        self.partyStartedTime = datetime.datetime(year = stime.tm_year, month = stime.tm_mon, day = stime.tm_mday,
    ...
    def loadDecorations(self):
        ...
        for decorBase in self.partyInfo.decors:
            self.decorationsList.append(Decoration(PartyGlobals.DecorationIds.getString(decorBase.decorId),
        ...
        if config.GetBool('show-debug-party-grid', 0):
            ...
            for i in range(len(self.grid)):
                for j in range(len(self.grid[i])):
                    ...
                    np.setScale(12)
                    np.setP(-90.0)
                    np.setPos(PartyUtils.convertDistanceFromPartyGrid(j, 0) - 6.0,
                              PartyUtils.convertDistanceFromPartyGrid(i, 1) - 6.0, 0.1)
    ...
    def getTimer(self, parent):
        ...
        timer['minute'] = DirectLabel(parent = parent, pos = TTLocalizer.DPtimerMinutePos, relief = None, text = '59',
                                      ...
                                      text_fg = (0.7, 0.3, 0.3, 1.0), scale = TTLocalizer.DPtimerMinute)
        timer['colon'] = DirectLabel(parent = parent, pos = TTLocalizer.DPtimerColonPos, relief = None, text = ':',
                                     text_align = TextNode.ACenter, text_font = timeFont,
                                     text_fg = (0.7, 0.3, 0.3, 1.0), scale = TTLocalizer.DPtimerColon)
        timer['second'] = DirectLabel(parent = parent, relief = None, pos = TTLocalizer.DPtimerSecondPos, text = '14',
                                      text_align = TextNode.ACenter, text_font = timeFont,
                                      text_fg = (0.7, 0.3, 0.3, 1.0), scale = TTLocalizer.DPtimerSecond)
        timer['textLabel'] = DirectLabel(parent = parent, relief = None, pos = (0.0, 0.0, 1.15),
    ...
    def doSpawnTitleText(self, text):
        ...
        self.titleText = OnscreenText.OnscreenText(text, fg = self.titleColor, font = ToontownGlobals.getSignFont(),
        ...
        seq = Task.sequence(Task.pause(0.1), Task.pause(6.0),
                            self.titleText.lerpColorScale(Vec4(1.0, 1.0, 1.0, 1.0), Vec4(1.0, 1.0, 1.0, 0.0), 0.5),
