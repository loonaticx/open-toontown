...
class PartyCatchActivityToonSD(StateData.StateData):
    ...
    def __init__(self, avId, activity):
        ...
        self.fsm = ClassicFSM.ClassicFSM('CatchActivityAnimFSM-%s' % self.avId, [
            State.State('init', self.enterInit, self.exitInit, ['notPlaying', 'normal', 'rules']),
    ...
    def enterRules(self):
        if self.isLocal:
            ...
            self.rulesPanel = MinigameRulesPanel('PartyRulesPanel', self.activity.getTitle(),
    ...
    def enterEatFruit(self, fruitModel, handNode):
        ...
        self.eatIval = Sequence(Parallel(WaitInterval(duration), Sequence(
            ...
            Func(fruitModel.hide))), Func(self.fsm.request, 'normal'), name = self.toon.uniqueName('eatingIval'))
        self.eatIval.start()
    ...
    def enterFallForward(self):
        ...
        self.fallFwdIval = Sequence(
            ActorInterval(self.toon, animName, startTime = startFrame / newRate, endTime = pauseFrame / newRate,
                          playRate = playRate), WaitInterval(duration / 2.0),
            ActorInterval(self.toon, animName, startTime = pauseFrame / newRate, endTime = totalFrames / newRate,
                          playRate = playRate), FunctionInterval(resume))
