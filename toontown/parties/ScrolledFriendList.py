...
class ScrolledFriendList(DirectScrolledList):
    ...
    def __init__(self, parent, gui, clickCallback = None, makeItemsCheckBoxes = False):
        ...
        self.gui = gui
        self.scrollSpeed = 1
        DirectScrolledList.__init__(self, parent = parent, relief = None, incButton_image = (
        self.gui.find('**/inviteButtonDown_up'), self.gui.find('**/inviteButtonDown_down'),
        self.gui.find('**/inviteButtonDown_rollover')), incButton_relief = None, incButton_pos = (0.0, 0.0, -0.03),
            ...
            self.gui.find('**/inviteButtonUp_rollover')), decButton_relief = None, decButton_pos = (0.0, 0.0, 0.02),
                                    decButton_image3_color = Vec4(0.6, 0.6, 0.6, 0.6), itemFrame_relief = None,
                                    forceHeight = 0.084, numItemsVisible = 8, items = [],
                                    incButtonCallback = self.scrollButtonPressed,
                                    decButtonCallback = self.scrollButtonPressed, itemFrame_pos = (0.0, 0.0, -0.01))
    ...
    def addFriend(self, name, id):
        if self.makeItemsCheckBoxes:
            ...
            widget = DirectCheckButton(relief = None, scale = 0.1, boxBorder = 0.08,
                                       boxImage = (uncheckedImage, checkedImage, None), boxImageScale = 10.0,
                                       boxRelief = None, text = name, text_align = TextNode.ALeft, text_scale = 0.7,
                                       text_pos = (-3.7, -0.25), command = self.clickCallback,
                                       indicator_pos = (-4.8, 0.0, 0.0))
            widget['extraArgs'] = [widget]
        else:
            widget = DirectLabel(relief = None, text = name, text_align = TextNode.ALeft, text_pos = (-0.6, 0.0, 0.0),
                                 scale = 0.055)
