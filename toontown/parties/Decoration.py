...
class Decoration(NodePath):
    ...
    def __init__(self, name, x, y, h):
        ...
        if self.name == 'CakeTower':
            ...
            collisionMesh.hide()
            self.globeSphere = CollisionSphere(collisionMesh.getBounds().getCenter(),
            ...
            soundIval = SoundInterval(self.decSfx, node = self.hydra, listenerNode = base.localAvatar,
                                      volume = PartyGlobals.DECORATION_VOLUME, cutOff = PartyGlobals.DECORATION_CUTOFF,
            ...
            soundIval = SoundInterval(self.decSfx, node = self.tubeCog, listenerNode = base.localAvatar,
                                      volume = PartyGlobals.DECORATION_VOLUME, cutOff = PartyGlobals.DECORATION_CUTOFF,
                                      duration = animIvalDur)
            self.animSeq = Parallel(animIval, soundIval)
            self.animSeq.loop()
    ...
    def startAnimSeq(self):
        if self.animSeq:
            self.animSeq.finish()
        if self.startAnim == 1:
            self.animSeq = Sequence(LerpHprInterval(self.decorationModel, 3.0,
                                                    Vec3(random.randint(0, 5), random.randint(0, 5),
                                                         random.randint(0, 5))), Wait(0.05), Func(self.startAnimSeq))
            self.animSeq.start()
