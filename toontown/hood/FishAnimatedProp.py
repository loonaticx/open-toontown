...
class FishAnimatedProp(AnimatedProp.AnimatedProp):
    ...
    def __init__(self, node):
        ...
        self.splashSfxList = (
        loader.loadSfx('phase_4/audio/sfx/TT_splash1.ogg'), loader.loadSfx('phase_4/audio/sfx/TT_splash2.ogg'))
        self.node = self.fish
        ...
        self.track = Sequence(FunctionInterval(self.randomizePosition), Func(self.node.unstash),
                              Parallel(self.fish.actorInterval('jump'),
                                       ...
                                       Sequence(Wait(1.14), Func(self.splash.play),
                                                SoundInterval(randomSplash, volume = 0.8, node = self.node))), Wait(1),
                              Func(self.node.stash), Wait(4 + 10 * random.random()), name = self.uniqueName('Fish'))
