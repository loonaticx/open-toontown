...
class BeanRecipeGui(DirectFrame):
    ...
    def createJellyBeanBox(self, beanIndex, xPos, zPos):
        ...
        xAdj = 0.03
        zAdj = 0.03
        newBox = DirectButton(parent = self, pos = (xPos + xAdj, 0, zPos + zAdj), geom = DGG.getDefaultDialogGeom(),
                              geom_scale = (0.1, 1.0, 0.1), geom_color = geomColor, scale = (0.5, 1, 0.5),
                              relief = None, state = state, command = command, extraArgs = [beanIndex],
                              text = TTLocalizer.BeanColorWords[beanIndex], text_pos = (0.0, 0.1), text_scale = 0.07,
                              text_fg = Vec4(0, 0, 0, 0), text1_fg = Vec4(0, 0, 0, 1), text2_fg = Vec4(0, 0, 0, 1),
                              text3_fg = Vec4(0, 0, 0, 0), clickSound = None, pressEffect = 0)
    ...
    def createEmptyBeanBox(self, xPos, zPos):
        ...
        newBox = DirectButton(parent = self, pos = (xPos + xAdj, 0, zPos + zAdj), geom = DGG.getDefaultDialogGeom(),
                              geom_scale = (0.1, 1.0, 0.1), geom_color = geomColor, scale = (0.5, 1, 0.5),
                              relief = None, state = state, command = command, text = '', text_pos = (0.0, 0.1),
                              text_scale = 0.07, text_fg = Vec4(0, 0, 0, 0), text1_fg = Vec4(0, 0, 0, 1),
                              text2_fg = Vec4(0, 0, 0, 1), text3_fg = Vec4(0, 0, 0, 0), clickSound = None,
                              pressEffect = 0)
