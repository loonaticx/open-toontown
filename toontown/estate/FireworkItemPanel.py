...
class FireworkItemPanel(DirectFrame):
    ...
    def __init__(self, itemName, itemNum, *extraArgs):
        ...
        DirectFrame.__init__(self, image = DGG.getDefaultDialogGeom(), image_color = (0.75, 0.75, 0.75, 1),
                             image_scale = (0.25, 0, 0.25), relief = None)
        self.initialiseoptions(FireworkItemPanel)
        self.load()
        return

    def load(self):
        self.picture = DirectButton(parent = self, image = (
        DGG.getDefaultDialogGeom(), DGG.getDefaultDialogGeom(), DGG.getDefaultDialogGeom()), relief = None,
                                    command = self.__launchFirework, extraArgs = [self.type],
        ...
        self.quantityLabel = DirectLabel(parent = self.picture, relief = None, pos = (0, 0, 0.0), scale = 0.45,
                                         text = self.name, text_scale = 0.6, text_fg = (0, 0, 0, 1),
                                         text_pos = (0, -.14, 0), text_font = nameFont, text_wordwrap = panelWidth)
        return
