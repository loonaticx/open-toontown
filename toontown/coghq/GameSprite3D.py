...
class GameSprite:
    ...
    def warningBump(self):
        ...
        track = Sequence(Wait(num1 * 0.1), LerpPosInterval(self.nodeObj, num2 * 0.1, Point3(0.0, 0.0, 0.5)),
    ...
    def shake(self):
        ...
        curr = self.nodeObj.getPos()
        dest = Point3(0 + curr[0], 0 + curr[1], 1.0 + curr[2])
        track = Sequence(LerpPosInterval(self.nodeObj, num2 * 0.1, Point3(0.0, 0.0, 0.25)),
                         LerpPosInterval(self.nodeObj, num1 * 0.1, Point3(0.0, 0.0, 0.0)))
        track.start()
    ...
    def deathEffect(self):
        ...
        track = Sequence(Parallel(
            ProjectileInterval(self.spriteBase, startVel = Vec3(-20.0 + notNum3 * 40.0, -20.0 + num3 * 40.0, 30),
                               duration = 0.5 + num2 * 1.0, gravityMult = 2.0),
            LerpColorScaleInterval(self.spriteBase, duration = 0.5 + num2 * 1.0,
                                   startColorScale = GameSprite.colorList[self.colorType],
                                   colorScale = GameSprite.disolveList[self.colorType])), Func(self.removeDelay),
                         Func(self.delete))
        track.start()
    ...
    def wildEffect(self):
        ...
        self.delayRemove = 1
        self.canCollide = 0
        track = Sequence(Parallel(LerpScaleInterval(self.spriteBase, 1.0, 1.5, startScale = 1.0),
