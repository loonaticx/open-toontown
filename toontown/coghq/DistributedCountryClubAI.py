...
class DistributedCountryClubAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def generate(self):
        ...
        for i in range(self.layout.getNumRooms()):
            room = DistributedCountryClubRoomAI.DistributedCountryClubRoomAI(self.air, self.countryClubId, self.doId,
                                                                             self.zoneId, self.layout.getRoomId(i),
                                                                             i * 2, self.avIds, self.battleExpAggreg)
        ...
        for room in self.rooms:
            if room.entType2ids['elevatorMarker']:
                for markerId in room.entType2ids['elevatorMarker']:
                    ...
                    newElevator = DistributedClubElevatorAI.DistributedClubElevatorAI(self.air, self.doId, self,
                                                                                      self.avIds, marker.doId)
    ...
    def startNextFloor(self):
        ...
        countryClub = DistributedCountryClubAI(self.air, self.countryClubId, countryClubZone, floor, self.avIds,
