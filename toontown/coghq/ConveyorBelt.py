...
class ConveyorBelt(BasicEntities.NodePathEntity):
    ...
    def initBelt(self):
        ...
        for i in range(self.numTreads):
            ...
            mp.setupCopyModel('conv%s-%s' % (self.getParentToken(), i), treadModel, self.floorName,
    ...
    def start(self):
        ...
        for i in range(self.numTreads):
            ...
            ival = Sequence()
            if periodsToEnd != 0:
                ival.append(LerpPosInterval(self.treads[i], duration = treadPeriod * periodsToEnd,
                                            pos = Point3(0, startY + self.numTreads * self.treadLength, 0),
                                            startPos = Point3(0, startY + i * self.treadLength, 0), fluid = 1))

            def dumpContents(tread = self.treads[i]):
                tread.releaseLocalToon()

            ival.append(Sequence(Func(dumpContents),
                                 Func(self.treads[i].setPos, Point3(0, startY + self.numTreads * self.treadLength, 0))))
            if periodsFromStart != 0:
                ival.append(LerpPosInterval(self.treads[i], duration = treadPeriod * periodsFromStart,
                                            pos = Point3(0, startY + i * self.treadLength, 0),
                                            startPos = Point3(0, startY, 0), fluid = 1))
