...
class DistributedStageAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def generate(self):
        ...
        for i in range(self.layout.getNumRooms()):
            room = DistributedStageRoomAI.DistributedStageRoomAI(self.air, self.stageId, self.doId, self.zoneId,
        ...
        for room in self.rooms:
            if room.entType2ids['elevatorMarker']:
                for markerId in room.entType2ids['elevatorMarker']:
                    ...
                    newElevator = DistributedElevatorFloorAI.DistributedElevatorFloorAI(self.air, self.doId, self,
    ...
    def startNextFloor(self):
        ...
        Stage = DistributedStageAI(self.air, self.stageId, StageZone, floor, self.avIds, self.layoutIndex,
                                   self.battleExpAggreg)
