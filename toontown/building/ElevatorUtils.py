...
def getLeftOpenInterval(distObj, leftDoor, type):
    ...
    leftOpenInterval = LerpPosInterval(leftDoor, openTime, openPos, startPos = closedPos, blendType = 'easeOut',
...
def getRightOpenInterval(distObj, rightDoor, type):
    ...
    rightOpenInterval = LerpPosInterval(rightDoor, openTime, openPos, startPos = closedPos, blendType = 'easeOut',
...
def getOpenInterval(distObj, leftDoor, rightDoor, openSfx, finalOpenSfx, type = ELEVATOR_NORMAL):
    ...
    if finalOpenSfx:
        sound = Sequence(SoundInterval(openSfx, duration = openDuration, volume = sfxVolume, node = leftDoor),
...
def getLeftCloseInterval(distObj, leftDoor, type):
    ...
    closedPos = getLeftClosePoint(type)
    openPos = getLeftOpenPoint(type)
    leftCloseInterval = LerpPosInterval(leftDoor, closeTime, closedPos, startPos = openPos, blendType = 'easeOut',
...
def getRightCloseInterval(distObj, rightDoor, type):
    ...
    closedPos = getRightClosePoint(type)
    openPos = getRightOpenPoint(type)
    rightCloseInterval = LerpPosInterval(rightDoor, closeTime, closedPos, startPos = openPos, blendType = 'easeOut',
...
def getCloseInterval(distObj, leftDoor, rightDoor, closeSfx, finalCloseSfx, type = ELEVATOR_NORMAL):
    ...
    sfxVolume = ElevatorData[type]['sfxVolume']
    if finalCloseSfx:
        sound = Sequence(SoundInterval(closeSfx, duration = closeDuration, volume = sfxVolume, node = leftDoor),
...
def getRideElevatorInterval(type = ELEVATOR_NORMAL):
    ...
    if type in (ELEVATOR_VP,
                ...
                ELEVATOR_CJ,
                ELEVATOR_BB):
        ival = Sequence(Wait(0.5),
                        LerpPosInterval(camera, 0.5, Point3(0, yValue, zMin), startPos = Point3(0, yValue, zMid),
                                        blendType = 'easeOut'),
                        LerpPosInterval(camera, 0.5, Point3(0, yValue, zMid), startPos = Point3(0, yValue, zMin)),
                        Wait(1.0),
                        LerpPosInterval(camera, 0.5, Point3(0, yValue, zMax), startPos = Point3(0, yValue, zMid),
                                        blendType = 'easeOut'),
                        LerpPosInterval(camera, 1.0, Point3(0, yValue, zMid), startPos = Point3(0, yValue, zMax)))
    else:
        ival = Sequence(Wait(0.5), LerpPosInterval(camera, 0.5, Point3(0, 14, 3.8), startPos = Point3(0, 14, 4),
                                                   blendType = 'easeOut'),
                        LerpPosInterval(camera, 0.5, Point3(0, 14, 4), startPos = Point3(0, 14, 3.8)), Wait(1.0),
                        LerpPosInterval(camera, 0.5, Point3(0, 14, 4.2), startPos = Point3(0, 14, 4),
                                        blendType = 'easeOut'),
