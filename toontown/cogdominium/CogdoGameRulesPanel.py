...
class CogdoGameRulesPanel(StateData.StateData):
    ...
    def load(self):
        ...
        self.gameTitleText = DirectLabel(parent = self.frame, text = self.gameTitle,
                                         scale = TTLocalizer.CRPgameTitleText, text_align = TextNode.ACenter,
                                         text_font = getSignFont(), text_fg = (1.0, 0.33, 0.33, 1.0),
                                         pos = TTLocalizer.CRPgameTitleTextPos, relief = None)
        ...
        self.playButton = DirectButton(parent = self.frame, relief = None, geom = (
        minigameGui.find('**/buttonUp'), minigameGui.find('**/buttonDown'), minigameGui.find('**/buttonHover')),
                                       pos = (0.74, 0, 0.1), scale = (4.2, 5, 5), command = self.playCallback)
