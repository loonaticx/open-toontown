...
class DistributedCogdoInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __handleUnexpectedExit(self, toonId):
        ...
        if len(self.toons) == 0:
            ...
            if self.fsm.getCurrentState().getName() == 'Resting':
                ...
                self.fsm.requestFinalState()
                self._disCleanupTask = taskMgr.doMethodLater(20, self._cleanupAfterLastToonWentDis,
                                                             self.uniqueName('discleanup'))
    ...
    def setAvatarJoined(self):
        ...
        if self.toons.count(avId) == 0:
            self.air.writeServerEvent('suspicious', avId,
                                      'DistributedCogdoInteriorAI.setAvatarJoined from toon not in %s.' % self.toons)
    ...
    def enterElevator(self):
        ...
        self.timer.startCallback(
            BattleBase.ELEVATOR_T + ElevatorData[ELEVATOR_NORMAL]['openTime'] + BattleBase.SERVER_BUFFER_TIME,
        ...
        if self._game:
            ...
            if score:
                ...
                if self._rewardedLaff:
                    self.air.writeServerEvent('CogdoLaffReward', list(self._rewardedLaff.keys()),
            ...
            if self._penaltyLaff:
                self.air.writeServerEvent('CogdoLaffPenalty', list(self._penaltyLaff.keys()),
    ...
    def enterBarrelRoomIntro(self):
        ...
        if self._wantBarrelRoom and not self.isBossFloor(self.currentFloor):
            ...
            self.brBarrier = ToonBarrier.ToonBarrier('waitBrIntroDone', self.uniqueName('waitBrIntroDone'), self.toons,
    ...
    def enterBarrelRoomReward(self):
        if self._wantBarrelRoom and not self.isBossFloor(self.currentFloor):
            ...
            self.brBarrier = ToonBarrier.ToonBarrier('waitBrRewardDone', self.uniqueName('waitBrRewardDone'),
    ...
    def __createFloorBattle(self):
        if self.isBossFloor(self.currentFloor):
            ...
            bossBattle = 0
        self.battle = DistributedCogdoBattleBldgAI.DistributedCogdoBattleBldgAI(self.air, self.zoneId,
    ...
    def enterReservesJoining(self):
        self.__resetResponses()
        self.timer.startCallback(
            ElevatorData[ELEVATOR_NORMAL]['openTime'] + SUIT_HOLD_ELEVATOR_TIME + BattleBase.SERVER_BUFFER_TIME,
    ...
    def enterReward(self):
        ...
        for v in victors:
            ...
            if tuple:
                ...
                if toon:
                    self.notify.info('Reward State: toonId:%d laff:%d/%d get ready for the victors to come outside' % (
