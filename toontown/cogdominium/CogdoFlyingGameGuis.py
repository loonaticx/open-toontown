...
class CogdoFlyingProgressGui(DirectFrame):
    ...
    def update(self):
        for toon, marker in list(self._toonMarkers.items()):
            progress = clamp((toon.getY() - self._levelStartY) / self._levelDistance, self._levelStartY,
                             self._levelEndY)
...
class CogdoFlyingFuelGui(DirectFrame):
    ...
    def _initModel(self):
        ...
        self.bladeNumberLabel = DirectLabel(parent = self.propellerHead, relief = None, pos = (
        Globals.Gui.FuelNumBladesPos2D[0], 0, Globals.Gui.FuelNumBladesPos2D[1]),
                                            scale = Globals.Gui.FuelNumBladesScale, text = str(len(self.activeBlades)),
                                            text_align = TextNode.ACenter, text_fg = (0.0,
                                                                                      0.0,
                                                                                      -0.002,
                                                                                      1),
    ...
    def _initIntervals(self):
        ...
        self._spinIval = LerpFunctionInterval(self.propellerMain.setR, fromData = 0.0, toData = -360.0,
