...
class CogdoGameMessageDisplay:
    ...
    def updateMessage(self, message = '', color = None, transition = 'fade'):
        ...
        if message == '':
            if transition in ('fade', 'blink'):
                self.transitionInterval.append(LerpFunc(self.messageLabel.setAlphaScale, fromData = 1.0, toData = 0.0,
            ...
            if transition == 'fade':
                self.transitionInterval.append(LerpFunc(self.messageLabel.setAlphaScale, fromData = 0.0, toData = 1.0,
                                                        duration = CogdoGameConsts.MessageLabelFadeTime,
                                                        extraArgs = []))
    ...
    def showMessageTemporarily(self, message = '', duration = 3.0, color = None):
        ...
        taskMgr.doMethodLater(duration, self.updateMessage, CogdoGameMessageDisplay.UpdateMessageTaskName,
                              extraArgs = [])
